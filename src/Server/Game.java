...
class Game {
	...
	public synchronized void invite(JSONObject msgJson) {
		...
		try {
			...
			for (int i=0;i<j.length();i++){
				...
				userList.put(j.getString(i),0);
			}
			this.howManyPlayers=userList.size();
			...
			System.out.println("hmp:"+this.howManyPlayers+"//"+players);
			pushScores();
			nextPlay();
		} catch (Exception e) {
	...
	public synchronized void vote(int agree){
		...
		this.countVoters++;
		System.out.println("HMP:"+howManyPlayers+"CV:"+this.countVoters+"CVS"+this.countVotes);
